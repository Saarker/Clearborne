...
#         print(sorted_titles)
#         if sorted_titles:
#             payload += f"**5 Most Recently Played Games:** \n"
#         for i in range(5):
#             payload += f" \n"
...
def get_response(user_input: str) -> dict:
    ...
    if user_input[:4] == "help":
        return misc.help(user_input[5:])
    elif user_input[:6] == "lookup":
        return profiles.lookup(user_input[7:])
    else:
        return Embed.from_dict({"description":"Something went wrong. Please try again."})